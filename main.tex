...
for k = 1:n_samples
    ...
    % Simular planta não linear por um passo
    dt = Ts;
...
hold on;
plot(t_sim/60, r_signal, 'r--', 'LineWidth', 1.5, 'DisplayName', 'Referência');
xlabel('Tempo [min]'); ylabel('CB [mol/l]');
...
title('Perturbação'); grid on;

% Função para simular um passo da planta não linear
function [CA_new, CB_new] = simular_passo_nao_linear(CA, CB, u, CAF, dt)
    % Integração usando Euler
...
% Calcular métricas para seguimento de referência
...
fprintf('Tempo de acomodação: %.2f min\n', t_settling);
